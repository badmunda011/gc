    ...
    if len(spam_tracker[user_id]) > MESSAGE_LIMIT:
        try:
            ...
            await context.bot.restrict_chat_member(
                ...
                until_date=current_time + SPAM_RESTRICTION_DURATION
            )
...
# Function to delete messages containing links or PDF files
async def delete_links_and_files(update: Update, context: CallbackContext):
    message = update.message
    user = message.from_user
    ...
    if message.document and message.document.file_name.endswith(".pdf"):
        try:
            # Delete the message with a PDF file
            await context.bot.delete_message(chat_id=message.chat_id, message_id=message.message_id)
            warning_text = f"⚠️ {user.mention_html()}, PDF files are not allowed."
            await context.bot.send_message(chat_id=message.chat_id, text=warning_text, parse_mode=ParseMode.HTML)
        except Exception as e:
            print(f"Failed to delete PDF file message: {e}")
...
# Handler for detecting links and PDF files
app_instance.add_handler(MessageHandler(filters.ALL & filters.ChatType.GROUPS, delete_links_and_files))
